...
base::tryCatch({
  affiliates <- DBI::dbGetQuery(con, "SELECT * FROM gold_bajo_monto")
...
affiliates <- affiliates %>%
  dplyr::select(num_ident,num_transa,Ahorro_Egreso,Ahorro_Ingreso,Virtual_Egreso,Virtual_Ingreso)

affiliates_t <- affiliates

# 3. Identify columns for Box-Cox transformation (from "num_transa" to "Virtual_Ingreso")
boxcox_columns <- base::seq(from = base::which(base::names(affiliates_t) == "num_transa"),
                            to = base::which(base::names(affiliates_t) == "Virtual_Ingreso"))
...
create_histogram_matrix(affiliates[,2:6], affiliates_t[,2:6])

# 5. Perform PCA (exclude first column if it's an identifier)
pca_results <- FactoMineR::PCA(affiliates_t[, -1], scale.unit = TRUE, ncp = 2, graph = FALSE)
# Optional scree plot:
...
create_fmsb_radar(radar_data, "Pam", 2)
create_fmsb_radar(radar_data, "Clara", 2)
create_fmsb_radar(radar_data, "Pam", 4)
create_fmsb_radar(radar_data, "Clara", 4)
...
clara_result <- fpc::claraCBI(centers, k = 4)
...
color_palette <- cluster_colors[base::as.character(base::sort(base::unique(clara_result$result$clustering)))]

centers_dist <- stats::dist(centers)

sil_widths <- cluster::silhouette(
  clara_result$partition,
  centers_dist
)

# Visualize silhouette scores
factoextra::fviz_silhouette(sil_widths) +
  ggplot2::scale_fill_manual(values = c("#F8766D", "#7CAE00", "#00BFC4", "#C77CFF")) +
  ggplot2::labs(
...
affiliates <- affiliates %>%
  dplyr::mutate(cluster = base::factor(clara_result$partition[centers_sample$cluster]))
...
create_cluster_radar_charts <- function(data, features_to_plot) {
  ...
  graphics::par(mfrow = c(2, 2), mar = c(1, 1, 3, 1))
...
create_cluster_radar_charts(
  data = affiliates,
  features_to_plot = c("num_transa", "Ahorro_Egreso", "Ahorro_Ingreso", 
                       "Virtual_Egreso", "Virtual_Ingreso")
)
...
# 11. Create summary_table: median of key features per cluster
summary_table <- affiliates %>%
  dplyr::group_by(cluster) %>%
  dplyr::summarize(
    dplyr::across(
      c("num_transa", "Ahorro_Egreso", "Ahorro_Ingreso", "Virtual_Egreso", "Virtual_Ingreso"),
...
knitr::kable(summary_table, 
             caption = "Median Statistics by Cluster")
...
writexl::write_xlsx(affiliates, "Clients/0002_Coopcentral/Model_2022_2023_2024/Results/SARLAFT_Coopcentral_Segmentacion_Reporte_Bajo_Monto.xlsx")
base::save.image("Clients/0002_Coopcentral/Model_2022_2023_2024/Results/results_Bajo_Monto.RData")
