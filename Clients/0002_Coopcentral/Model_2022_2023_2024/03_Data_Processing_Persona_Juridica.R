...
base::tryCatch({
  affiliates <- DBI::dbGetQuery(con, "SELECT * FROM gold_persona_juridica")
...
affiliates <- affiliates %>%
  dplyr::select(-c(jurisdiccion_3_Ingreso,jurisdiccion_3_Egreso))
...
pca_results <- FactoMineR::PCA(affiliates_t[, -1], scale.unit = TRUE, ncp = 14, graph = FALSE)
pca_results$eig
# Optional scree plot:
...
validation <- clValid::clValid(
  ...
  maxitems = 4000,  # Adjust based on your scores matrix size
  method = "ward"
)
...
create_fmsb_radar(radar_data, "Hierarchical", 2)
...
writexl::write_xlsx(affiliates, "Clients/0002_Coopcentral/Model_2022_2023_2024/Results/SARLAFT_Coopcentral_Segmentacion_Persona_Juridica.xlsx")
base::save.image("Clients/0002_Coopcentral/Model_2022_2023_2024/Results/results_Persona_Juridica.RData")
